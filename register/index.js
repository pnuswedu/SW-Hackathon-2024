$(document).ready(function() {
    ...
    $("#id").on("propertychange change keyup paste input", function() {
        ...
        if(!check_id(currentVal)) {
            ...
            $('label[for = "id"]').text("ID는 영어, 숫자로만 입력가능하며 8자리 이상 16자리 이하여야합니다.");
        }
        else {
    ...
    $("#class").on("propertychange change keyup paste input", function() {
        ...
        if(!check_class(currentVal)) {
            $(this).removeClass('is-valid');
            $(this).addClass('is-invalid');
            $('label[for = "class"]').text("유효하지 않은 이름입니다.");
        }
        else {
            $(this).removeClass('is-invalid');
            $(this).addClass('is-valid');
            $('label[for = "class"]').text("유효한 이름입니다.");
        }
    });
    ...
    $("#stu_num").on("propertychange change keyup paste input", function() {
        ...
        if(!check_stu_num(currentVal)) {
            $(this).removeClass('is-valid');
            $(this).addClass('is-invalid');
            $('label[for = "stu_num"]').text("유효하지 않은 이름입니다.");
        }
        else {
            $(this).removeClass('is-invalid');
            $(this).addClass('is-valid');
            $('label[for = "stu_num"]').text("유효한 이름입니다.");
        }
    });
});
